...
im = Image.open('D:/cs498ML/hw5/polarlights.jpg')
...
for i in range(image.shape[0]):
    for j in range(image.shape[1]):
        newPicture[i][j] = np.dot(np.asarray(newPixel[i * image.shape[1] + j]), 10.0)



img = Image.fromarray(newPicture, "RGB")
img.save('D:/cs498ML/hw5/polarlights_50.jpg')
